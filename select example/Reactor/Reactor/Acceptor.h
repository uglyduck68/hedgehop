...
namespace X1
{
	...
	public:
		...
		X1_HANDLE 	GetHandle() const
		{
			return m_h;
		}

		/// called when input event occur (e.g., connection or data)
		int		HandleRead(X1_HANDLE h);

		/// called when output events are possible
		int 	HandleWrite(X1_HANDLE h);

		int 	HandleException(X1_HANDLE h);

		int 	HandleClose(X1_HANDLE h = INVALID_HANDLE, EVENT_MASK e = ALL_EVENTS_MASK);
