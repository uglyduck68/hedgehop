...
/************************************************************************************
 ...
 # History
	[2013.11.21] Byungung Min(byungung@mdstec.com)
	 v1.0 - NeoDDS Engine 연동을 위한 LBTS_DDS_LIBRARY v1.0 생성
	[2014.11.17] NeoDDS(neodds@mdstec.com)
	 v2.0 - LBTS_DDS_LIBRARY v4.0 호환
 *************************************************************************************/
...
#define _REMOVE_ELEM( type, p_ar, i_oldsize, i_pos )								\
    do																				\
    {																				\
        ...
        if( i_oldsize > 1 )															\
        {																			\
            (p_ar) = (type)realloc( p_ar, ((i_oldsize) - 1) * sizeof( *(p_ar) ) );	\
        }																			\
        else																		\
        {																			\
            FREE( p_ar );															\
...
public:
	...
	ReturnCode_t GetParticipantHandles(InstanceHandleSeq Seq)
	{
		if(!m_pParticipant) return 0;
		ReturnCode_t ret = m_pParticipant->get_discovered_participants(m_pParticipant,&Seq);
		return ret;
	}
	ReturnCode_t GetParticipantData(ParticipantBuiltinTopicData* Data,InstanceHandle_t handle){
		if(!m_pParticipant) return 0;
		return m_pParticipant->get_discovered_participant_data(m_pParticipant,Data,handle);
	}
	~CDDSDomain() 
	{
		Finalize();
	...
	static void on_data_available(DataReader* reader) {
		...
		if(fseq.i_seq)
		{
			...
			while(fseq.i_seq)
			{
				...
				_REMOVE_ELEM(Foo**, fseq.pp_foo, fseq.i_seq, 0);
				_REMOVE_ELEM(SampleInfo**, sSeq.pp_sample_infos, sSeq.i_seq, 0);
			}
		}
	}
};
...
public:
	...
	bool CreateWriter(std::string strQosName) 
	{
		...
		if (CreateTopic()) 
		{
			...
			memset(&writerQos, 0, sizeof(DataWriterQos));
	...
	bool CreateReader(std::string strQosName, CALLBACK_FUNC pCallBackFunc) 
	{
		...
		if (CreateTopic()) 
		{
			...
			memset(&readerQos, 0, sizeof(DataReaderQos));
	...
	inline void Start_DDS_Service(void(*routine)(void*))
	{

	}

	inline void End_DDS_Service()
	{
		dds_quit();
	}
	template<int nDomainID>
	inline ReturnCode_t GetParticipantHandles(InstanceHandleSeq& Seq){
		if(is_dds_quit()) return 0;
		InstanceHandleSeq InstanceHandleSeq = INIT_INSTANCESEQ;
		ReturnCode_t ret = CDDSDomain<nDomainID>::singleton().GetParticipantHandles(InstanceHandleSeq);
		printf("Get Participant Handles Call %d RetCode %d\n",InstanceHandleSeq.i_seq,ret);
		Seq = InstanceHandleSeq;
		return ret;
	}
	template<int nDomainID>
	inline ReturnCode_t GetParticipantData(ParticipantBuiltinTopicData* Data,InstanceHandle_t handle){
		if(is_dds_quit()) return 0;
		return CDDSDomain<nDomainID>::singleton().GetParticipantData(Data,handle);
	}
}
...
#define LDDS_START(callbackFunction)			LBTS_DDS::Start_DDS_Service(callbackFunction)
#define LDDS_END()								LBTS_DDS::End_DDS_Service()
#define LDDS_GETPARTICIPANTHANDLE(nDomainID,Seq)	LBTS_DDS::GetParticipantHandles<nDomainID>(Seq)
#define LDDS_GETDISCOVERYDATA(nDomainID,Data,handle)		LBTS_DDS::GetParticipantData<nDomainID>(Data,handle)
#endif
