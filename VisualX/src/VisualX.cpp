...
#include <EnLib2.h>		//* include DDS library header
#include "OPVEnum.h"
...
void CVisualX::CreateIntroViewport()
{
	...
	if( m_IntroViewport )
	{
		...
		m_IntroCamera->setAspectRatio( Ogre::Real(m_IntroViewport->getActualWidth()) / 
			Ogre::Real(m_IntroViewport->getActualHeight()));
	}
}
...
bool CVisualX::frameStarted( const Ogre::FrameEvent &evt )
{
...
bool CVisualX::frameRenderingQueued(const Ogre::FrameEvent& evt)
{
	...
	{
		m_snIntroTarget->yaw(Ogre::Degree(evt.timeSinceLastFrame * 30));
...
{
	static Ogre::Real	waveFreq	= 0.028f;
	static Ogre::Real waveAmp		= 1.8f;
	...
	if(currentTechnique)
	{
		...
		if(mActivePass)
		{
			...
			Ogre::GpuProgramParametersSharedPtr activeParameters = mActiveVertexParameters;
...
/**
* @function		CallbackFullInformation
* @brief		DDS receiver callback function
*/
