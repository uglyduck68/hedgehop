	...
	static void on_data_available(DataReader* reader) {
		...
		if(fseq.i_seq)
		{
			...
			while(fseq.i_seq)
			{
				...
				_REMOVE_ELEM(SampleInfo**, sSeq.pp_sample_infos, sSeq.i_seq, 0); // 20140220
			}
		}
	}
};
...
public:
	...
	bool CreateWriter(std::string strQosName) 
	{
		...
		if (CreateTopic()) 
		{
			...
			writerQos.resource_limits.max_instances = 1000;
#endif
	...
	bool CreateReader(std::string strQosName, CALLBACK_FUNC pCallBackFunc) 
	{
		...
		if (CreateTopic()) 
		{
			...
			readerQos.resource_limits.max_instances = 1000;
#endif
