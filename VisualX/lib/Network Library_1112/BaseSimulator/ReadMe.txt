================================================================================
    MFC 라이브러리 : BaseSimulator 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 BaseSimulator 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

BaseSimulator 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

BaseSimulator.vcxproj
...
BaseSimulator.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CBaseSimulatorApp 응용 프로그램 클래스가 선언되어 있습니다.

BaseSimulator.cpp
    이는 응용 프로그램 클래스 CBaseSimulatorApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\BaseSimulator.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 BaseSimulator.rc에 의해 포함됩니다.

res\BaseSimulator.rc2
...
res\Toolbar.bmp
    ...
    초기 도구 모음과 상태 표시줄은 CMainFrame 클래스에서 생성됩니다. 이 도구 모음 비트맵을 리소스 편집기로 편집하고, BaseSimulator.rc의 IDR_MAINFRAME TOOLBAR 배열을 업데이트하여 도구 모음 단추를 추가합니다.
/////////////////////////////////////////////////////////////////////////////

응용 프로그램 마법사에서 문서 형식과 뷰를 한 개씩 만듭니다.

BaseSimulatorDoc.h, BaseSimulatorDoc.cpp - 문서
    이 파일에는 CBaseSimulatorDoc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 CBaseSimulatorDoc::Serialize를 통해 파일 저장 및 로드를 구현합니다.

BaseSimulatorView.h, BaseSimulatorView.cpp - 문서 뷰
    이 파일에는 CBaseSimulatorView 클래스가 포함됩니다.
    CBaseSimulatorView 개체는 CBaseSimulatorDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 BaseSimulator.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.

Resource.h
    새 리소스 ID를 정의하는 표준 헤더 파일입니다. Microsoft Visual C++에서 이 파일을 읽고 업데이트합니다.

BaseSimulator.manifest
