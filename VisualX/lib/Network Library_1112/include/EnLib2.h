...
#include "msg/CMS/DDS_CMS_module.hpp"
	...
	inline RefPtr<ICommandInterface> Initialize( const std::function<void(SUB*)>& Func,const std::string& SUBQOS, const std::string& PUBQOS,unsigned char CSCI,unsigned char CSC,unsigned short TopicID)
	{
		return CDDSCommand<nDomain,SUB,PUB,Type>::CreateCommand(Func,SUBQOS,PUBQOS);
	}
	template<int nDomain,typename PUB>
	inline RefPtr<ICommandInterface> Initialize(const std::string& QOS = "EventReliable",unsigned char CSCI = 0,unsigned char CSC = 0,unsigned short TopicID = 0)
	{
	...
	inline RefPtr<ICommandInterface>& Initialize( const std::function<void(SUB*)>& Func,const std::string& SUBQOS = "EventReliable",const std::string& PUBQOS = "EventReliable",unsigned char CSCI = 0,unsigned char CSC = 0,unsigned short TopicID = 0)
	{
	...
	inline RefPtr<ICommandInterface>& Initialize( const std::function<void(SUB*)>& Func,const std::string& QOS = "EventReliable",unsigned char CSCI = 0,unsigned char CSC = 0,unsigned short TopicID = 0 )
	{
	...
	inline CDDSCommand<1,T>* GetCommand()
	{
		...
		return static_cast<CDDSCommand<1,T>*>(Interface.Get());
	}
