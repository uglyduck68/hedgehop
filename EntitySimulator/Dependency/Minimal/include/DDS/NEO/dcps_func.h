...
extern "C" {
...
#ifdef DDS_147_JWKIM_141204_MULTI_DOMAIN	//jwkim : 각 builtin reader를 domainID로 가져오도록 수정
...
#else
	DataReader* get_builtin_participant_reader(DomainId_t domainID);
	DataReader* get_builtin_topic_reader(DomainId_t domainID);
	DataReader* get_builtin_publisher_reader(DomainId_t domainID);
	DataReader* get_builtin_subscriber_reader(DomainId_t domainID);
	...
	- DIDE-1
	***************************************************************/
	void get_builtin_topic_typecode(void* p_topic_typecode, void* p_cache);
