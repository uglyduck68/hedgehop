...
public:
	...
	Gorilla::Rectangle*      rectMiniMap;
	Gorilla::Rectangle*			rectSym;	// symbol on minimap
	...
	App() : mTimer(0), mTimer2(0), mNextUpdate(0)
	{
		...
		// [20150315] Sean, add some test codes for minimap HUD
		mLayerMiniMap	= mScreen->createLayer(1);

		rectMiniMap		= mLayerMiniMap->createRectangle(LEFT, TOP, WIDTH, HEIGHT);

		// "coco" is sprite name that is defined in .gorilla file.
		// So you *MUST* make texture atlas using minimap.
		rectMiniMap->background_image("coco");

		rectSym			= mLayerMiniMap->createRectangle(LEFT+WIDTH/2, TOP+HEIGHT/2, 2, 2);
		rectSym->background_colour(Gorilla::Colours::AliceBlue);
	}
	...
	bool frameStarted(const Ogre::FrameEvent& evt)
	{
		...
		// [20150315] Sean, add some test codes to move rectangle
		Vector2	pos(Math::RangeRandom(LEFT+0, LEFT+WIDTH), Math::RangeRandom(TOP+0, TOP+HEIGHT));

		rectSym->position(pos);
