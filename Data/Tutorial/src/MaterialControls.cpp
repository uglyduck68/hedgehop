...
void MaterialControls::addControl(const Ogre::String& params)
{
	...
	sprintf(caTemp, "%s: %s, %s, %s, %s, %s, %s", __FUNCTION__, vecparams[0].c_str(), vecparams[1].c_str(), vecparams[2].c_str(), vecparams[3].c_str(), vecparams[4].c_str(), vecparams[5].c_str());

	Ogre::LogManager::getSingleton().logMessage( caTemp );
...
void loadMaterialControlsFile(MaterialControlsContainer& controlsContainer, const Ogre::String& filename)
{
	...
	Ogre::LogManager::getSingleton().logMessage("section name: " + secName + ", material name: " + materialName );
                ...
                for (i = settings->begin(); i != settings->end(); ++i)
                {
                    ...
                    if (typeName == "control")
					{
	Ogre::LogManager::getSingleton().logMessage("controlsContainer[" + Ogre::StringConverter::toString(idx) + "].addControl: " + dataString + " ***END***");
...
void loadAllMaterialControlFiles(MaterialControlsContainer& controlsContainer)
{
	...
	{
		// print material name
Ogre::LogManager::getSingleton().logMessage("controlsFileNameIterator is " + *controlsFileNameIterator);
