...
#define PIbsend(type,buffer,length,to,datatype) \
     Egida_Send(to, type, (char *) buffer, length, datatype)
...
#define PIbrecv(type,buffer,length,datatype)  \
     { char *__p4lbuf=0;\
       ...
       __P4LEN = length; \
       __p4lbuf = 0; \
       Egida_Receive(&__P4FROM, &__P4TYPE, &__p4lbuf, &__P4LEN);\
       memcpy(buffer, __p4lbuf, __P4LEN); \
       /* p4_msg_free(__p4lbuf); \ */ \
       Egida_LogDeliveredMsg(__p4lbuf); \
     }

#define PInprobe(type) (__P4TYPE=type,__P4FROM= -1,\
        Egida_Msgs_Avail(&__P4FROM, &__P4TYPE))

#define PIbroadcast(type,buffer,length,datatype) \
     Egida_Broadcast(type, (char *)buffer, length,datatype)
