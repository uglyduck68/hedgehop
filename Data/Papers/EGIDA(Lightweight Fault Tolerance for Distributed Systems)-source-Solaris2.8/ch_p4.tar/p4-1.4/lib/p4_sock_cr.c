...
struct p4_procgroup *pg;
{
    ...
    /* Change made by Sriram 7/29/98 */
    create_reg_server(serv_port, &reg_serv_port);
	...
	for (i=1, pe = pg->entries+1; i < pg->num_entries; i++, pe++)
	{
	  ...
	  /*
	    rm_fd = net_create_slave(serv_port,serv_fd,
				     ...
				     pe->username);
				     */
	    rm_fd = net_create_slave(reg_serv_port,serv_fd,
				     pe->host_name,
				     pe->slave_full_pathname,
				     pe->username);
    ...
    /* Added by Sriram 7/29/98 */
    /* Tell the reg server about process 0's listener port */
...
P4VOID net_resend_slave_info(pe, outfile, rm_fd, rm_num)
struct p4_procgroup_entry *pe;
char *outfile;
int rm_fd, rm_num;
{
	...
	switch (type)
	{
	  ...
	  case REMOTE_SLAVE_INFO:
	    ...
	    pidx = install_in_proctable(rm_num,port,slave_pid,
					...
					msg.machine_type,remote_switch_port);
	    */
	    UpdateProcTable(rm_num, port, slave_pid, pe->host_name);
	    pidx = rm_num;
            p4_dprintfl(90, "net_slave_info: adding connection to %d (%d) \n",
		        pidx,rm_num);
...
/* -- This is to handle sighup which gets sent when parent terminates */
P4VOID setup_sighup_handler()
{
  void sighup_handler(int);

  SIGNAL_P4(SIGHUP, sighup_handler);
  SIGNAL_P4(SIGINT, sighup_handler);
}

static void sighup_handler(int signo)
{
  printf("signal = %d received, pid = %d\n", signo, getpid());
  sleep(1);
  return;
}
