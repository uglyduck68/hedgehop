...
char **msg;
{
	...
	if (tmsg == NULL)
	{
	    if ((*req_from >= 0) && (p4_DeadConnection(*req_from)))
	      return -1;
...
#   if defined(CAN_DO_SOCKET_MSGS)
	...
	/* 
	 * I don't think we need to do anything here in case of failure.  
	 * The function call socket_msgs_available() will succeed the
	 * first time failure is detected; on the next call, the
	 * function will return false.  So, no harm done?
	 */
    }
#   endif
...
#   endif
	...
	for (i = 0; i < p4_global->num_in_proctable; i++)
		...
		p4_error( "Found a dead connection while looking for messages",
			   i );
		*/
		p4_dprintfl(70, "Found a dead connection while looking for messages", i);
		}
	    }
	}
...
P4VOID re_initialize_msg_queue(mq)
struct p4_msg_queue *mq;
{
    (P4VOID) p4_moninit(&(mq->m), 1);
    p4_lock_init(&(mq->ack_lock));
