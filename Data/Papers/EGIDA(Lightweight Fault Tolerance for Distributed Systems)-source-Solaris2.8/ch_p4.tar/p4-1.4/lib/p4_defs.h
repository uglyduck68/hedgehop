...
#define NUMAVAILS 8
#define MAX_HASH_VALUES 7  /* keep it an odd number */

struct p4_avail_buff {
    int size;			/* size of message portion */
    struct p4_msg *buff;
  /* changed by Sriram -- 7/8/99 */
    struct p4_msg *hashTable[MAX_HASH_VALUES];
};

#ifdef P4_ORIGINAL_CODE
struct p4_avail_buff {
    int size;			/* size of message portion */
    struct p4_msg *buff;
  /* changed by Sriram -- 8/3/98 */
    struct p4_msg *used;
  /* Added by Sriram -- 11/25/98 */
    struct p4_msg *used_tail;
};
#endif
...
/* 
 * when a process fails and rejoins, the bm re-distributes the
 * process table to every process in the system.  to an operational
 * process, the table is sent to the listener.  for the listener to
 * know what kind of message it is, a dummy message with a header
 * "PROC_TABLE_BEGIN" is sent by bm to the listener.
 */
#define PROC_TABLE_BEGIN        16
#define PROC_TABLE_ENTRY        17
#define PROC_TABLE_END          18
#define SYNC_MSG                19
