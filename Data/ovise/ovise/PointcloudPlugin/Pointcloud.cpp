...
CPointcloud::CPointcloud( const std::string& Name,
                          const std::string& Resourcegroup,
                          const std::size_t  VNumber,
                          float*             VArray,
                          float*             CArray )
	: mNumVertices( VNumber )
{
	...
	mVertexBuffer =
			...
			mDeclaration->getVertexSize(
				0 ), mMsh->sharedVertexData->vertexCount,
			Ogre::HardwareBuffer::
			HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE );
	// Upload the vertex data to the card
	mVertexBuffer->writeData( 0,
		mVertexBuffer->getSizeInBytes(), VArray, true );

	if( CArray != NULL )
	{
		// Create 2nd buffer for colors
		mDeclaration->addElement( 1,
			0,
			Ogre::VET_COLOUR,
			Ogre::VES_DIFFUSE );
		mColorBuffer =
				...
				Ogre::VertexElement::getTypeSize(
					Ogre::VET_COLOUR ),
				mMsh->sharedVertexData->vertexCount,
				Ogre::HardwareBuffer::
				HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE );
		...
		for( std::size_t i=0, k=0;
		     i < mNumVertices * 3 && k < mNumVertices;
		     i+=3, k++)
		{
		...
		mColorBuffer->writeData( 0,
			mColorBuffer->getSizeInBytes(), colours, true );
		delete[] colours;
	}

	// / Set vertex buffer binding so buffer 0 is bound to our vertex buffer
	Ogre::VertexBufferBinding* bind =
	        mMsh->sharedVertexData->vertexBufferBinding;
	...
	mMsh->load();
	mMsh->_setBounds( Ogre::AxisAlignedBox( minx, miny, minz, maxx, maxy,
			maxz ) );
}
...
void CPointcloud::PrepareBuffers( std::size_t VertexSize )
{
	if( VertexSize != mNumVertices )
	{
		...
		mMsh->sharedVertexData->vertexBufferBinding->setBinding( 0,
			mVertexBuffer );
		mMsh->sharedVertexData->vertexBufferBinding->setBinding( 1,
			mColorBuffer );
	}
}
...
void CPointcloud::UpdateVertexPositions( std::size_t VNumber, float* Vertices )
{
	...
	maxx = maxy = maxz = -1000000000;
	mVertexBuffer->writeData( 0,
		mVertexBuffer->getSizeInBytes(), Vertices, true );
...
void CPointcloud::UpdateVertexColours( std::size_t CNumber, float* Colors )
{
	...
	{
		mColorBuffer->writeData( 0,
			mColorBuffer->getSizeInBytes(), Colors, true );
	}
	else
	{
