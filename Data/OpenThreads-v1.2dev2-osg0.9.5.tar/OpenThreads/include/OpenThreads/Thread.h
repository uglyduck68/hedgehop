...
public:
	 ...
	 * Join just call pthread_join API in pthread environment.
     *
     *  @return 0 if normal, -1 if errno set, errno code otherwise.
     */
    int join();
	...
	/**
	 * Join(int msec) work well in Windows.
	 * But *NOT* works in ptreads environment 
	 * because Join(int msec) just call join function in pthread env.
	 */
	int Join(int msec);	///< newly added by Sean
	int Join()
	{
		return join();
	}
	...
	// compatable interface with X1 package
...
class OPENTHREAD_EXPORT_DIRECTIVE ThreadMQ :public Thread {
...
*/
template<typename L>
class ThreadVMQ : public Thread
{
public:
	ThreadVMQ(void);
	virtual ~ThreadVMQ(void);
...
protected:
	MsgQueue<MsgQueueItem, L>	m_MsgQueue;
};

/**
 * exception class for compatable with X1 package
 */
