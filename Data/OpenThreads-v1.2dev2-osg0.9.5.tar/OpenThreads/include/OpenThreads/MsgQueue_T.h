...
struct MsgQueueItem
{
	...
	*/
	...
	int	Set(void* pData, int Len)
	{
		...
		//
		// I try to decrease the memory usage But that try has
		// some defeats that lose consistency.
		//
 ...
 *				You also use MutexNull class for single thread.
 * @param		@L is lock type that is one of Mutex or MutexNull.
 * @note		
 * FIXME: use lock-free algorithm of MPMC.
 */
...
public:
	...
	*				But this function will be never used.
	*/
	void			Pop()
	{
		Guard<L>	scopeLock(this->m_Mutex);
