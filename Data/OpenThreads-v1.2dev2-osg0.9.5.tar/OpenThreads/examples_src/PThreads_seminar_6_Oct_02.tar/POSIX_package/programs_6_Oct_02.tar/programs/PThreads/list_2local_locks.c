...
/*
  ...
  This program allocates two mutexes per element, one to protect the
  next-pointer and following member, the other to protect the salary of
...
/*
cc -o list_2local_locks list_2local_locks.c -L. -R. -g -lpthread -lthread -pthread_extensions -lposix4
*/
...
{person_t *p;
 ...
 p->name = name;
 pthread_mutex_init(&(p->next_lock), NULL);
 pthread_mutex_init(&(p->salary_lock), NULL);
 pthread_mutex_lock(&(people.next_lock));
 p->next = people.next;
 people.next = p;
 pthread_mutex_unlock(&(people.next_lock));
}

pthread_mutex_lock_conflicts(pthread_mutex_t *people_lock)
{if (pthread_mutex_trylock(people_lock) == EBUSY)
      {count_conflicts(1);
...
{person_t *p = &people, *p1, *p2;
 ...
 pthread_mutex_lock(&(p->next_lock));
 p1 = p->next;
 while(p1 != NULL)
   {if (strcmp(p1->name, name) == 0)
      /* Found! Return *previous* person, Still holding p->lock ! */
      {count_finds(1); return(p);}
    pthread_mutex_lock_conflicts(&(p1->next_lock));
    pthread_mutex_unlock(&(p->next_lock));
    p = p1;
    p1 = p->next;
  }
 pthread_mutex_unlock(&(p->next_lock));
...
{person_t *p1;
 ...
 pthread_mutex_unlock(&(p->next_lock)); /* Unlock here!! */
 p1->salary++;
 delay(RAISE_DELAY, 0);
...
{person_t  *p1;
 ...
 p1 = p->next;
 pthread_mutex_lock(&(p1->next_lock));
 p->next = p1->next;
 pthread_mutex_unlock(&(p->next_lock));

 DEBUG(printf("Killing: (%s %d)\n", p1->name, p1->salary))
 pthread_mutex_lock(&(p1->salary_lock));
 pthread_mutex_unlock( &(p1->next_lock));
 pthread_mutex_destroy(&(p1->next_lock));
 pthread_mutex_unlock( &(p1->salary_lock));
...
{int i;
 ...
 pthread_mutex_init(&(people.next_lock), NULL);
 ...
 printf(
 "list_2local_locks(L_DELAY: %d R_DELAY: %d N_PEOPLE: %d\n N_FRIENDS: %d N_ETHREADS: %d N_FTHREADS: %d)\n",
