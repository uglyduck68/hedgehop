...
/*
  ...
  a mutex to protect *all* the salaries.
...
/*
cc -o list_global_rw list_global_rw.c -L. -R. -g -lpthread -lthread -pthread -lposix4
*/
...
pthread_mutex_t         salary_lock = PTHREAD_MUTEX_INITIALIZER;
sem_t                   barrier;
...
{int i;
 ...
 printf(
 "list_global_rw(L_DELAY: %d R_DELAY: %d N_PEOPLE: %d\n N_FRIENDS: %d N_ETHREADS: %d N_FTHREADS: %d)\n",
