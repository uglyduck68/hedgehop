...
/*
cc -o stop_queue_problem stop_queue_problem.c -L. -R. -g -lpthread -lthread -lthread_extensions -lposix4
*/
...
pthread_attr_t          attr;
sem_t                   barrier;
int                     GET_DELAY = 20, PROCESS_DELAY = 250;
int                     stop = FALSE;
int                     N_PROD = 2, N_CONS = 2;
int                     n_produced = 0, n_processed = 0;
...
{request_t *request;
 while(TRUE)
   {request = get_request();
    ...
    while (length >= 10)
      {delay(20, 0);            /* Leave in!  Makes lost wakeup more likely. */
