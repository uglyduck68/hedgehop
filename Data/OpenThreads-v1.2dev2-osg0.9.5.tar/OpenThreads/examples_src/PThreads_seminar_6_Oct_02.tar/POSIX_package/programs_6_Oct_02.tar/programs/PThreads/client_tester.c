...
cc -o client_tester client_tester.c -L. -R. -D__sun -D__sun2_6 -g -lpthread -lthread -lposix4  -lthread_extensions -lnsl -lsocket
*/
...
{int                    i, j, sockfd = (int) arg;
 ...
 for (j=1; TRUE; j++)
   {int         nbytes;
    ...
    if (nbytes < 0) {perror("Client read failed");}
    if (nbytes != MESSAGE_SIZE)
      {printf("Client read failed! Read: %d bytes.\n", nbytes);
   if (stop) return;
      }
