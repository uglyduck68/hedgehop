...
/*
  ...
  This program allocates one mutex per element to protect the
  next-pointer, following member, and the salary of
  the following member.  It is *vital* that you know exactly what each
...
/*
cc -o list_local_lock list_local_lock.c -L. -R. -g -lpthread -lthread -pthread_extensions -lposix4
*/
...
{person_t *p;
 ...
 pthread_mutex_lock(&(people.lock));
 p->next = people.next;
 people.next = p;
 pthread_mutex_unlock(&(people.lock));
}
...
{person_t *p = &people, *p1, *p2;
 ...
 while(p1 != NULL)
   {if (strcmp(p1->name, name) == 0)
    ...
    pthread_mutex_lock_conflicts(&(p1->lock));    
...
void give_raise(person_t *p)
{
  ...
  p->next->salary++;
  delay(RAISE_DELAY, 0);
  pthread_mutex_unlock(&(p->lock));             /* Unlock here */
}
...
{int i;
 ...
 pthread_mutex_init(&(people.lock), NULL);
 ...
 printf(
 "list_local_lock(L_DELAY: %d R_DELAY: %d N_PEOPLE: %d\n N_FRIENDS: %d N_ETHREADS: %d N_FTHREADS: %d)\n",
