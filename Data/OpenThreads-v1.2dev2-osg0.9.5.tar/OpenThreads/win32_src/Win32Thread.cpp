/**
 * [20140312] Sean
 *	- change CreatThread and ExitThread to _beginthreadex and _endthreadex
 */
...
int Thread::start() {
	...
	unsigned int/*long*/ ID;	// _beginthreadex return thread ID of unsigned int type

	/**
	 * refer to http://msdn.microsoft.com/en-us/library/windows/desktop/ms682453(v=vs.85).aspx
	 * A thread in an executable that calls the C run-time library (CRT) [LIBCMT.LIB] should use 
	 * the _beginthreadex and _endthreadex functions for thread management rather than CreateThread and ExitThread;
	 */
//    pd->tid = CreateThread(NULL,pd->stackSize,ThreadPrivateActions::StartThread,static_cast<void *>(this),0,&ID);

	pd->tid	= (HANDLE)_beginthreadex(NULL, pd->stackSize, ThreadPrivateActions::StartThread, static_cast<void *>(this), 0, &ID);

	pd->uniqueId = (int)ID;

    if((uintptr_t)pd->tid == -1L/*INVALID_HANDLE_VALUE*/) {	// _beginthreadex return -1L on error
		return -1;
    }
